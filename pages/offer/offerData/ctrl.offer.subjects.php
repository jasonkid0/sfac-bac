...
if (isset($_POST['add'])) {
    ...
    if (!empty($_POST['day']) and !empty($_POST['time']) and !empty($_POST['room'])) {
        ...
        if ($count == 0) {
            // insert Data
            $query = $db->query("INSERT INTO tbl_schedules (class_code, subj_id, faculty_id, day, time, room, section, acad_year, semester, special_tut) VALUES ('$subj_code', '$subj_id', '$instructor', '$day' , '$time', '$room', '$section', '$_SESSION[AC]', '$_SESSION[S]', '$special_tut[0]')") or die($db->error);
