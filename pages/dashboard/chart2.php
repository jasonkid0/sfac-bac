...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <?php
        ...
        $get_ay = mysqli_query($db,"SELECT * FROM tbl_acadyears JOIN tbl_semesters WHERE NOT academic_year = '' AND NOT semester = 'Summer' ORDER BY academic_year ASC, semester ASC");
        while ($row1 = mysqli_fetch_array($get_ay)) {
            array_push($ay_array, "".$row1['semester']." - ". $row1['academic_year']);
            ...
            WHERE tbl_schoolyears.ay_id = '$row1[academic_year]' AND tbl_schoolyears.sem_id = '$row1[semester]' AND remark IN ('Approved')");
            ...
            LEFT JOIN tbl_courses ON tbl_courses.course_id = tbl_schoolyears.course_id
            LEFT JOIN tbl_departments ON tbl_departments.department_id = tbl_courses.department_id
            WHERE tbl_schoolyears.ay_id = '$row1[academic_year]' AND tbl_schoolyears.sem_id = '$row1[semester]' AND remark IN ('Approved') AND status IN ('Old') ");
            $count = mysqli_fetch_array($deptdata);
            array_push($student_array, $count['count']);
            } elseif($dep == "New") {
                $deptdata = mysqli_query($db, "SELECT COUNT(sy_id) AS count FROM tbl_schoolyears
            LEFT JOIN tbl_courses ON tbl_courses.course_id = tbl_schoolyears.course_id
            LEFT JOIN tbl_departments ON tbl_departments.department_id = tbl_courses.department_id
            WHERE tbl_schoolyears.ay_id = '$row1[academic_year]' AND tbl_schoolyears.sem_id = '$row1[semester]' AND remark IN ('Approved') AND status IN ('New') ");
            $count = mysqli_fetch_array($deptdata);
            array_push($student_array, $count['count']);
            } else {
                $deptdata = mysqli_query($db, "SELECT COUNT(sy_id) AS count FROM tbl_schoolyears
            LEFT JOIN tbl_courses ON tbl_courses.course_id = tbl_schoolyears.course_id
            LEFT JOIN tbl_departments ON tbl_departments.department_id = tbl_courses.department_id
            WHERE tbl_schoolyears.ay_id = '$row1[academic_year]' AND tbl_schoolyears.sem_id = '$row1[semester]' AND tbl_departments.department_id = '$dep' AND remark IN ('Approved')");
            $count = mysqli_fetch_array($deptdata);
            array_push($student_array, $count['count']);
            }
            /////////////////////////////////////////////////////////////////////////////
